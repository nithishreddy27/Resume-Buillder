            ...
            <div className='container w-[210mm] h-[297mm] min-w-[210mm] bg-white flex'>
                <div className='first w-[85mm] h-[297mm] bg-emerald-700'>
                    ...
                    <div className='firstone w-[85mm] relative top-[-100px]'>
                        ...
                        <div className="dob pl-10 pt-3 text-white"><i className='bx bxs-calendar pr-4 text-lg text-white'></i><span className="text-base relative bottom-[3px] text-white">{details.personal.dob}</span></div>
                        <div className="phone pl-10 pt-3 text-white"><i className='bx bxs-phone pr-4 text-lg text-white'></i><span class="text-base relative bottom-1 text-white">{details.personal.phone}</span></div>
                        <div className="mail pl-10 pt-3 pb-2 text-white"><i className='bx bxs-envelope pr-4 text-lg text-white'></i><span class="text-base relative bottom-1 text-white">{details.personal.email}</span></div>
                        </div>
                        ...
                        {
                                details.education.map(item=>(
                                    <p key={item.institution} className="pl-10 pr-5 pt-5"><span className="font-medium text-white">{item.institution}</span><span className='text-white'> in </span><span className="font-medium text-white">{item.fieldOfStudy}<br/>({item.startDate} to {item.endDate})</span><br/><i className='bx bxs-graduation text-white'></i><span className='text-white'> {item.typeOfDegree} in {item.fieldOfStudy} ({item.gpa})</span></p>
                                ))
                        }
                        </div>
                        )}
                        ...
                        {
                                details.certifications.map(item=>(
                                    <p  key={item.title} className="pl-10 pr-5 pt-1 text-white"><i className='bx bxs-square text-xs pr-3 text-white'></i><span className='text-white'>{item.title} from {item.issuer}</span></p>
                                ))
                        }
                        </div>
                        )}
                    ...
                    {
                                details.skills.map(item=>(
                                    <span key={item.name} className="pr-2 text-white">{item.name} </span>
                                ))
                    }
                    </div>
                    </div>
                ...
                <div className='second bg-emerald-100 w-[125mm] h-[297mm]'>
                        ...
                        <div className="career-objective">
                            ...
                            <p className="pl-10 pr-5 pt-2 text-xs">{details.objective}</p>
                        </div>
                        )}
                        ...
                        <div className="experience">
                            ...
                            {
                                details.work.map(item=>(
                                    <p key={item.work} className="pl-10 pr-5 pt-2"><span className="font-medium text-lg pr-3">{item.designation} in {item.company}</span><br></br>({item.from} to {item.to})<br/><Link href={item.website}>{item.website}</Link><br></br><span className='text-sm'>{item.summary.data}</span></p>
                                ))
                            }
                        </div>
                        )}
                        ...
                        <h2 className="text-center text-xl font-sans font-bold pt-2">A W A R D S</h2>
                            {
                                details.awards.map(item=>(
                                    <p key={item.name} className="pl-10 pr-5 pt-1"><i className='bx bxs-award pr-1' ></i><span className="font-medium">{item.name}</span> from {item.awarder}</p>
                                ))
                            }
                        </div>
                        )}
